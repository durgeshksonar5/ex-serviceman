...
(function ($) {
  ...
  // Generic form submission handler (Added by QA)
  // Generic form submission handler
  if ($("form").length > 0) {
    $("form").on("submit", function (e) {
      e.preventDefault();

      // Exclude search forms from the alert
      if (
        $(this).closest(".search_form").length > 0 ||
        $(this).closest(".header-search").length > 0 ||
        $(this).closest(".hamburger_search").length > 0
      ) {
        return;
      }

      // Simulate success for contact/newsletter forms
      alert("This is a demo form. Your message would be sent successfully!");
      this.reset();
    });
  }
})(jQuery);
